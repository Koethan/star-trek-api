...
const connectionString = 'mongodb+srv://ethan:startrek%402023@cluster0.8prfup9.mongodb.net/?retryWrites=true&w=majority'

app.use(cors())
app.use(express.json()) // this helps facilitate communication back and forth
...
MongoClient.connect(connectionString)
    ...
    app.get('/api/:alienName', (request,response) => {  //the : tells express that what comes after is the query parameter
        const aliensName = request.params.alienName.toLowerCase() //this tells express the get the parameter 
            infoCollection.find({name: aliensName}).toArray()//go into the database look for the object that has the name from the parameter
                .then(results => {
                    console.log(results)
